...
class TextIDE {
    ...
    handle_arrow(key: string, cursor_position: row_col): row_col {
        ...
        switch (key) {
            case 'ArrowLeft':
                ...
                if (cursor_position.col > 0) return { row: cursor_position.row, col: cursor_position.col - 1 };
            ...
            case 'ArrowRight':
                this.preffered_col = null;
                if (cursor_position.row === lines_total - 1 && cursor_position.col === last_line_len) return cursor_position;
    ...
    reset_active_row() {
        ...
        if (selected_row) selected_row.style.boxShadow = 'none';
    }
